  ...
  const handleAnalyzeSubmission = async () => {
    ...
    try {
      ...
      setActiveResult({
        ...
        confidence: data.execution_trace?.confidence_score || `${(data.confidence * 100).toFixed(1)}%`,
        resolution: data.execution_trace?.parameters_applied?.spatial_alignment_crs || "0.65m (Cartosat-2S)",
      ...
      setChatHistory((prev) => [
        ...prev,
        { role: "agent", text: "⚠️ Analysis error: Ensure the FastAPI server is running on port 8000." }
      ]);
    } finally {
      setIsLoading(false);
    }
